...
fn main() {
	...
	let _ = new_builder(&openssl_path, &nginx_path)
		...
		.whitelisted_function("SSL_send_fatal_alert")
		.whitelisted_var("SSL_AD_UNRECOGNIZED_NAME")
		.whitelisted_function("CRYPTO_BUFFER_new")
		.whitelisted_function("^SSL_set_(ocsp_response|signed_cert_timestamp_list|chain_and_key)$")
		.whitelisted_function("^SSL_CTX_set_(select_certificate|cert)_cb$")
		.whitelisted_function("SSL_CTX_set_session_id_context")
	...
	let _ = new_builder(&openssl_path, &nginx_path)
		...
		.whitelisted_var("^NGX_(ERROR|AGAIN|DECLINED)$")
